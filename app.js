...
function getSchedule(dayIdx) {
  ...
  return [...base, ...added, ...hotelActivitiesForDay(day.date), ...activityBookingsForDay(day.date)];
}
...
// Returns virtual schedule items for independently-confirmed activity bookings
// on `date` (concerts, tours, restaurants, etc.).  These are backed by
// trip.bookings — fromBookingId lets the panel render an ✎ Edit button and
// write time changes back to the booking.  autoGenerated items (hotel
// check-in/out) are handled separately by hotelActivitiesForDay().
function activityBookingsForDay(date) {
  if (!date || !trip.bookings) return [];
  // Map booking icon → ITEM_TYPES key so concerts get 🎵, food gets 🍽️ etc.
  const iconToType = { calendar: 'event', food: 'food', shopping: 'shopping' };
  // Avoid duplicating anything already emitted by hotelActivitiesForDay
  const alreadyLinked = new Set(hotelActivitiesForDay(date).map(i => i.id));
  return trip.bookings
    .filter(b =>
      b.category === 'activity' &&
      !b.autoGenerated &&
    ...
    .map(b => ({
      id:            b.id,
      type:          iconToType[b.icon] || 'attraction',
      title:         b.title,
      time:          b.activityTime || null,
      ampm:          null,
      status:        'confirmed',
      notes:         b.notes        || '',
      confirmation:  b.confirmation || '',
...
function renderBookingFilters() {
  ...
  // Place dropdown — only show if 2+ places have bookings (exclude autoGenerated)
  const usedPlaceKeys = [...new Set(trip.bookings.filter(b => !b.autoGenerated).map(b => b.colorKey).filter(Boolean))];
...
function renderBookings() {
  ...
  // Also exclude autoGenerated dependent activities (check-in/out etc.) — those
  // live in the day panel Schedule section, not the bookings list.
  let filtered = trip.bookings.filter(b => b.category !== 'todo' && !b.autoGenerated);
...
function openBookingEditor(idx) {
  ...
  openBookingPanel(`Edit ${catLabel}`, fields,
    (fd) => {
      ...
      if (booking.category === 'hotel') { syncBookingToStays(booking); syncHotelActivities(booking); }
      sortBookings();
      saveTrip();
      renderBookings();
      renderPlaces();
      showUndoToast('Booking updated', () => {
        ...
        if (snapshot.category === 'hotel') { syncBookingToStays(snapshot); syncHotelActivities(snapshot); }
        sortBookings();
        saveTrip();
        renderBookings();
        renderPlaces();
    ...
    () => {
      if (booking.category === 'hotel') { removeBookingStays(booking); removeHotelActivities(booking); }
...
function openNewBookingForm(category) {
  ...
  openBookingPanel(`New ${catLabel}`, fields,
    (fd) => {
      ...
      if (category === 'hotel') { syncBookingToStays(blank); syncHotelActivities(blank); }
      sortBookings();
      saveTrip();
      renderBookings();
      renderPlaces();
      showUndoToast('Booking added', () => {
        ...
        if (i >= 0) {
          if (blank.category === 'hotel') { removeBookingStays(blank); removeHotelActivities(blank); }
...
// ── Hotel check-in / check-out auto-activities ──────────────────────────────
// Each hotel booking gets two linked activity entries (check-in at 15:00,
// check-out at 12:00). These are regular editable activity records; the
// linkedHotelId + checkType fields let us sync date/title changes back to them
// and avoid duplicates.  activityTime is intentionally NOT synced — user may
// override the default after creation.

function syncHotelActivities(hotel) {
  if (!hotel || hotel.category !== 'hotel') return;
  // Ensure the hotel has an id — manually-added hotels may lack one
  if (!hotel.id) hotel.id = generateId();

  const linked  = trip.bookings.filter(b =>
    b.category === 'activity' && b.linkedHotelId === hotel.id
  );
  const existIn  = linked.find(b => b.checkType === 'in');
  const existOut = linked.find(b => b.checkType === 'out');

  // Propagate hotel changes to existing linked activities
  if (existIn) {
    existIn.title        = `Check-in · ${hotel.title}`;
    existIn.activityDate = hotel.checkIn  || existIn.activityDate;
    existIn.colorKey     = hotel.colorKey || existIn.colorKey;
    if (hotel.confirmation) existIn.confirmation = hotel.confirmation;
  }
  if (existOut) {
    existOut.title        = `Check-out · ${hotel.title}`;
    existOut.activityDate = hotel.checkOut || existOut.activityDate;
    existOut.colorKey     = hotel.colorKey || existOut.colorKey;
    if (hotel.confirmation) existOut.confirmation = hotel.confirmation;
  }

  // Create any still missing
  if (!existIn && hotel.checkIn) {
    trip.bookings.push({
      id: generateId(), category: 'activity', checkType: 'in',
      title: `Check-in · ${hotel.title}`,
      activityDate: hotel.checkIn, activityTime: '15:00',
      colorKey: hotel.colorKey || '', linkedHotelId: hotel.id,
      autoGenerated: true,
      address: hotel.address || '', confirmation: hotel.confirmation || '',
      notes: '', cost: '', url: hotel.url || '',
    });
  }
  if (!existOut && hotel.checkOut) {
    trip.bookings.push({
      id: generateId(), category: 'activity', checkType: 'out',
      title: `Check-out · ${hotel.title}`,
      activityDate: hotel.checkOut, activityTime: '12:00',
      colorKey: hotel.colorKey || '', linkedHotelId: hotel.id,
      autoGenerated: true,
      address: hotel.address || '', confirmation: hotel.confirmation || '',
      notes: '', cost: '', url: hotel.url || '',
...
function removeHotelActivities(hotel) {
  if (!hotel || !hotel.id) return;
  trip.bookings = trip.bookings.filter(b =>
    !(b.category === 'activity' && b.linkedHotelId === hotel.id)
  );
}
...
async function init() {
  ...
  // Migration v9: auto-generate check-in (3 pm) / check-out (noon) activity
  // entries for all existing hotel bookings that don't already have them.
  if ((trip.meta?.version || 1) < 9) {
    trip.bookings
      .filter(b => b.category === 'hotel' && b.id)
      .forEach(syncHotelActivities);
    trip.meta.version = 9;
    saveLocal();
  }

  // Migration v10: re-run for hotels that had id:null during v9 and were skipped.
  // syncHotelActivities now auto-assigns an id, and deduplication prevents duplicates.
  if ((trip.meta?.version || 1) < 10) {
    trip.bookings
      .filter(b => b.category === 'hotel')
      .forEach(syncHotelActivities);
    trip.meta.version = 10;
    saveLocal();
  }
